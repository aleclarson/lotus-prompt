...
var Event, KeyBindings, KeyEmitter, Promise, Type, canSync, caret, emptyFunction, fs, isType, log, readline, stripAnsi, type;
...
type.defineMethods({
  sync: function(options) {
    ...
    KeyEmitter._setupStream(this._stream);
  ...
  async: function(options) {
    ...
    this._async = Promise.defer();
    readline.emitKeypressEvents(this._stream);
    this._setLabel(options.label);
    this._open();
    this._stream.setRawMode(true);
    this._stream.on("keypress", listener = function(char, key) {
      return KeyEmitter._keypress(char, key);
    });
    this._async.listener = listener;
    return this._async.promise;
  },
  close: function(message) {
    ...
    if (this._async) {
      this._stream.removeListener("keypress", this._async.listener);
      this._async.resolve(this._close());
      return;
    }
    this._close();
  },
  ...
  _loopSync: function() {
    var buffer, length;
    buffer = Buffer(3);
    length = fs.readSync(this._stream.fd, buffer, 0, 3);
